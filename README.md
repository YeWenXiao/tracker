...
- **文件校验：** 上传接口添加 `validate_image()` 校验，限制文件类型（jpg/jpeg/png/bmp）和大小（10MB）。
- **并发安全：** `threading.Lock` 保护 target_info.json 读写，`_write_info` 使用临时文件 + `os.replace` 原子写入防断电损坏。
- **管理页面中文化：** 标题、按钮、统计面板、事件日志等全部改为中文；目标卡片显示权重/阈值标签并支持编辑。
- **回滚功能入口：** 管理页面新增历史快照面板，显示最近3个快照，支持一键回滚。

### Round 8 新增

- **模板特征预计算缓存：** 基于文件路径+修改时间+大小的 MD5 hash 缓存 ORB/SIFT/HSV/Edge 特征到 `.feature_cache/` 目录。二次启动时跳过特征计算，显著加速模板加载。cv2.KeyPoint 序列化为 tuple 存储。
- **API 请求限流：** 全局限流 30次/分钟，上传接口更严格 10次/分钟。基于内存的滑动窗口限流器，按客户端 IP 计数。超限返回 429 状态码。
- **健康检查端点：** `GET /api/health` 返回服务状态、运行时间、Python/OpenCV 版本、目标数量、活跃分组、特征缓存状态，以及关键组件（目标目录、配置文件、识别器）检查结果。
