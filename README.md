...
接收目标照片集 → RTSP 视频流中搜索目标 → 云台自动居中锁定 → 自动变焦放大至目标占画面 30-40%。

**核心特性：** 三状态机 (SEARCH→TRACK→TERMINAL) | HSV颜色搜索+YOLO辅助 | PID自适应云台 | 智能变焦（居中后放大） | <1秒找到目标 | Web监控

```bash
git checkout v1.8-realtime-track
python main.py                          # 全功能追踪
python main.py --no_gimbal --no_zoom    # 纯识别验证
```

详细说明见 [v1.8 分支 README](../../tree/v1.8-realtime-track)。
